#ifndef ASY_DRAWING_H
#define ASY_DRAWING_H
#include "complex_calc.h"
#include <fstream>
#include <string>
class draw_asy {
private:
	std::fstream asy_file;
public:
	draw_asy(std::string filename) {
		asy_file.open(filename,std::ios::in|std::ios::out|std::ios::trunc);
		asy_file<<R"(defaultpen(fontsize(10pt));
usepackage("amsmath");
usepackage("amssymb");
...
unitsize(10cm);)"<<"\n";
	...
	void draw(cline x,std::string colour="black") {
		if(x.is_line()) {
			complex dir=x.direction()/abs(x.direction());
			complex midpoint=dir*x.offset();
			complex point_a=midpoint+complex(1000)*dir*complex(0,1),point_b=midpoint-complex(1000)*dir*complex(0,1);
			asy_file<<"draw(Line(("<<real(point_a)<<", "<<imag(point_a)<<"), ("<<real(point_b)<<", "<<imag(point_b)<<")), "<<colour<<");\n";
		} else {
			asy_file<<"draw(circle(("<<real(x.centre())<<", "<<imag(x.centre())<<"), "<<x.radius()<<"), "<<colour<<");\n";
		}
	}
	void close() {
		asy_file<<"clip((-3,-3)--(-3,3)--(3,3)--(3,-3)--cycle);\n";
	}
};
#endif
